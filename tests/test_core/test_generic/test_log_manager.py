...
def sample_log():
    ...
    return Log(content=Note(message="test message"), loginfo=Note(level="INFO"))
...
def test_log_manager_capacity(sample_log):
    ...
    new_log = Log(content=Note(message="second message"), loginfo=Note(level="INFO"))
    manager.log(new_log)
...
def test_log_manager_file_naming(temp_dir):
    ...
    manager = LogManager(
        persist_dir=temp_dir, file_prefix="test_", use_timestamp=True, hash_digits=5
    )
...
async def test_log_manager_concurrent_logging():
    ...
    async def add_log(i):
        log = Log(content=Note(message=f"message {i}"), loginfo=Note(level="INFO"))
...
def test_log_manager_error_handling(temp_dir):
    ...
    invalid_dir.mkdir(parents=True, exist_ok=True)  # Create directory structure

    manager = LogManager(persist_dir=invalid_dir)

    # Should handle directory creation
    manager.log(Log(content=Note(message="test"), loginfo=Note(level="INFO")))
...
async def test_log_manager_async_context():
    ...
    manager = LogManager()
    log = Log(content=Note(message="test message"), loginfo=Note(level="INFO"))
