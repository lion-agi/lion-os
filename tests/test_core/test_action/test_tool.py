...
from lion.core.action.tool import Tool, func_to_tool
...
def test_func_to_tool_multiple():
    ...
    assert [t.function_name for t in tools] == ["example_func", "example_processor"]
...
def test_func_to_tool_validation():
    ...
    with pytest.raises(ValueError):
        func_to_tool([example_func, example_processor], parser=[example_parser])
