...
from lion.core.action.action_manager import ActionManager
from lion.core.action.function_calling import FunctionCalling
from lion.core.action.tool import Tool
from lion.core.communication.action_request import ActionRequest
...
async def test_match_tool_dict(populated_manager):
    ...
    with pytest.raises(ValueError):
        populated_manager.match_tool({"function": "invalid_func", "arguments": {}})
...
async def test_invoke(populated_manager):
    ...
    # Test with ActionRequest
    request = ActionRequest(function="helper_func", arguments={"x": 3, "y": "test"})
