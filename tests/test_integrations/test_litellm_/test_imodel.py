...
from lion.integrations.litellm_.imodel import RESERVED_PARAMS, iModel
...
def test_init_with_direct_api_key():
    ...
    model = iModel(api_key=direct_key)
...
def test_to_dict(model_instance, model_kwargs):
    ...
    expected = {
        ...
        "api_key": model_kwargs["api_key"],  # Should be the original env var name
    }
...
def test_from_dict(model_kwargs, monkeypatch):
    ...
    # Execute
    model = iModel.from_dict(model_kwargs)
...
async def test_invoke():
    # Setup
    model = iModel(model="gpt-4")
    mock_response = {"choices": [{"message": {"content": "Hello!"}}]}

    # Mock the acompletion method
    with patch.object(model, "acompletion", new_callable=AsyncMock) as mock_acompletion:
        mock_acompletion.return_value = mock_response

        # Execute
        result = await model.invoke(messages=[{"role": "user", "content": "Hi"}])
...
async def test_invoke_removes_reserved_params():
    # Setup
    model = iModel(
        model="gpt-4", invoke_action="test", instruction="test", clear_messages=True
    )
    mock_response = {"choices": [{"message": {"content": "Hello!"}}]}
...
def test_hash(model_instance):
    ...
    model2 = iModel(**model_instance.kwargs)
    assert hash(model_instance) == hash(model2)


def test_hash_equality():
    # Setup - use only hashable types for testing
    model1 = iModel(model="gpt-4", temperature=0.7)
    model2 = iModel(model="gpt-4", temperature=0.7)
    model3 = iModel(model="gpt-4", temperature=0.8)
